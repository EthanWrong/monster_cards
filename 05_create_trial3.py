...
Trial 3: Changed expected input of orig_card from a list to just a str
"""
...
def get_new_name(msg, title, orig_name=None, cat=card_cat):
...
def edit_card(card, orig_card):
    ...
    if choice == "Rename":  # edit name
        new_name = get_new_name(f"Rename {card[0]}", title, orig_card[0])
        if new_name:
            card[0] = new_name
