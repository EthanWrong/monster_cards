...
v6
- Reduced words in main menu buttons
- Replaced looping mechanics in main() with a while loop
- Added cat parameter to edit_card(), reshuffled cat parameter in
  get_new_name(), and removed default cat=card_cat parameter for future
  proofing
- Removed redundant return edit_card() statement in edit_card() if rename
v7
- Removed printing cards except in output_cards()
- Removed 'delete' button from REVIEW when reviewing a newly created card
- Fixed bug: If all cards were deleted from cat, errors in SEARCH and REVIEW
  would crash the program
v8
- Changed card name decoration from ** to == (i.e. ==Stoneling==)
